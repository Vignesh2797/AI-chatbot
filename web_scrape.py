...
class Ticket(object):
    ...
    def get_cheapest_ticket(page_contents, isReturn, departDate, returnDate):
        try:
            try:
                info = json.loads(
                    page_contents.find("script", {"type": "application/json"}).text
                )
            except:
                page = requests.get(Ticket.url)
                soup = BeautifulSoup(page.content, "html.parser")
                info = soup.find("script", {"type": "application/json"}).text
                
            ticket = {}
            ticket["url"] = Ticket.url
            ticket["isReturn"] = isReturn
            ticket["departDate"] = custom_to_date(departDate, "%d-%b-%Y")
            ticket["departureStationName"] = str(
                info["jsonJourneyBreakdown"]["departureStationName"]
            )
            ticket["arrivalStationName"] = str(
                info["jsonJourneyBreakdown"]["arrivalStationName"]
            )
            ...
            if isReturn:
                ...
                ticket["fareProvider"] = info["returnJsonFareBreakdowns"][0][
                    "fareProvider"
                ]
                ticket["returnTicketType"] = info["returnJsonFareBreakdowns"][0][
                    "ticketType"
                ]
                ticket["ticketPrice"] = info["returnJsonFareBreakdowns"][0][
                    "ticketPrice"
                ]
            else:
                ticket["fareProvider"] = info["singleJsonFareBreakdowns"][0][
                    "fareProvider"
                ]
                ticket["ticketPrice"] = info["singleJsonFareBreakdowns"][0][
                    "ticketPrice"
                ]
